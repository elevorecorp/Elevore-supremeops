            ...
            <div className="min-h-screen p-5 bg-black animate-in fade-in duration-700">
                ...
                <div className="max-w-md mx-auto space-y-5 pb-20">
                    ...
                    <div className="text-center space-y-2 pt-4">
                        ...
                        <p className="text-[9px] text-green-500 font-bold uppercase tracking-[0.4em]">Live Mission Hub</p>
                    </div>
                    {/* Urgency banner */}
                    ...
                    <div className="g p-6 text-center space-y-2">
                        ...
                        <p className="text-[9px] text-green-500 font-black uppercase pt-2">💸 Zelle: (407) 952-4228</p>
                    </div>
                    {/* Quote approval */}
                    {!job.approval_signature?(
                        <div className="g p-6 border border-amber-500/30 space-y-4">
                            <div className="text-center"><p className="text-[9px] font-black text-amber-500 uppercase tracking-widest mb-1">📝 Approve Your Quote</p><p className="text-[8px] text-slate-500 font-black italic">Sign below to confirm this service</p></div>
                            <SigPad onSave={saveApproval} label="Sign to approve quote"/>
                        </div>
                    ):(
                    ...
                    {job.status==='paid'&&(
                        <div className="space-y-2">
                            <button onClick={()=>window.open(GOOGLE_LINK)} className="w-full gold py-4 rounded-2xl font-black uppercase text-sm active:scale-95 transition-all">⭐ Leave a Google Review</button>
                            <button onClick={()=>{const l=`${window.location.origin}${window.location.pathname}?ref=${job.client_name?.replace(/\s/g,'_')}`;navigator.clipboard?.writeText(l);showToast("Link copied! 🎁");}} className="w-full bg-white/10 text-white py-4 rounded-2xl font-black uppercase text-sm active:scale-95 transition-all">🎁 Refer a Friend — Both Get $25 Off</button>
                        </div>
                    )}
        ...
        <div className="min-h-screen flex flex-col pb-32">
            ...
            <main className="max-w-xl mx-auto w-full p-4 space-y-5">
                ...
                <div className="flex gap-1 bg-black/40 p-1.5 rounded-2xl border border-white/5 shadow-xl overflow-x-auto no-sb">
                    {[['brief','☀️','Brief'],['intel','📊','Intel'],['agenda','📋','Logs'],['clients','🧬','DNA'],['drive','📁','Drive'],['deploy','⚡','New']].map(([v,e,l])=>(
            ...
            {/* BOTTOM NAV */}
            <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[92%] max-w-sm g p-2.5 flex justify-around items-center border border-white/10 shadow-[0_30px_60px_rgba(0,0,0,1)] z-[1000]">
                {[['brief','sun','amber'],['intel','bar-chart-2','green'],['agenda','shield-check','blue'],['clients','users','purple'],['drive','image','cyan'],['deploy','zap','amber']].map(([v,icon,c])=>(
                    <button key={v} onClick={()=>{if(v==='deploy'){setEditId(null);setState(INITIAL);setView('deploy');setDeployTab('identity');}else setView(v);}}
                        className={`p-2.5 rounded-xl flex-1 flex flex-col items-center gap-0.5 transition-all ${view===v?`text-${c}-400 bg-white/5`:'text-slate-600'}`}>
                        <i data-lucide={icon} className="w-4 h-4"></i>
                        <span className="text-[6px] font-black uppercase">{v==='deploy'?'New':v==='brief'?'AM':v}</span>
                    </button>
                ))}
            </nav>
        </div>
